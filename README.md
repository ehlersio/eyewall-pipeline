...
# Optional -- only social_posts.py uses these; without them it renders but doesn't publish
...
python ai_predictions.py --locale fr             # Pre-game predictions, French only
...
**Two bugs fixed when it was extracted.** Both files had their own copy, and both valued a **goal at a flat 1.0** -- its outcome, not its location -- the NHL twin of the PWHL bug below. The band values (0.20 / 0.07 / 0.03) were also roughly double the real per-band scoring rates. Together, 2025-26 league xG came to **21,527 against 8,915 actual goals, 2.41x**; fixing only the goal case still left 1.53x. A goal counted for as much as 5 high-danger shots, or 33 from distance, so any single line's or player's xG was dominated by whether they happened to score.

The values are each band's real goal rate, measured over 2025-26 (163,908 attempts, 8,915 goals) and checked against 2024-25 (166,849 / 8,650), which had no part in fitting them: high 0.0939 / 0.0923, medium 0.0613 / 0.0608, low 0.0301 / 0.0286. Summed over a season they give **0.998x** that season's goals and **1.022x** the holdout's. Team-level xG shares barely move (at most 0.21 points, median 0.07) -- the inflated goal values were spread roughly in proportion to scoring -- so the correction shows up in absolute scale and in small samples, not in team ranking. `rapm.py` re-exports `shot_xg`/`DANGER_XG` since `backtest_predictions.py` calls `rapm.shot_xg`. Both consumers recompute nightly (`run.py`), so no backfill was needed. Covered by `test_nhl_shot_xg.py`.

A caveat kept on purpose: a blocked shot's coordinates are where the *block* happened, not the shot, so blocked attempts land in closer bands than they belong in. They stay counted (the same choice `pwhl_shot_xg.py` makes), and the calibration absorbs the bias because the rates were measured with them in.
...
**Projection backtests (2026-09), read-only, not yet used by this module:**
...
**Empty-feed guard (2026-09-15):** ESPN's feed served 0 teams one night, where the night before had 86 rows across 27 teams (its per-team endpoints returned nothing either, so this was ESPN's side, not ours). `run()`'s delete-then-insert took that literally and wiped `player_injuries`, blanking every team's injury report in the app until the rows were restored from the previous day's `player_injury_history` snapshot. `run()` now compares what the feed returned against what's already stored: below `FEED_COLLAPSE_RATIO` (25%) of the stored rows, with at least `FEED_COLLAPSE_MIN_STORED` (20) rows to protect, it keeps the stored rows, writes no history snapshot for that day, and prints a `WARN`. Ordinary churn is nowhere near that (86 → 70 → 86 across that same week). An empty table is never protected — a first run still writes.
...
ESPN's feed goes back to at least 2015 (backfilled 2026-09: 13,477 entries for 2015–2024, 852 of them trades). ESPN's `/teams` only lists current franchises, so `HISTORICAL_TEAMS` adds Arizona (ESPN id 24, stored as `ARI` — the tri-code the NHL's draft records use); before that, every Arizona entry was skipped and other teams' trades with Arizona had no counterparty.
...
`player_id` comes from `PlayerIndex`: the `players` table plus every NHL skater and goalie since 2014-15 (NHL stats API, `skater/summary` and `goalie/summary` per season, fetched each run; if that fails the stage falls back to `players` alone). Exact normalized name when it belongs to one player, else the one player with the same last name whose first name is a prefix of the other's ("Alex" / "Alexander Kerfoot"); ambiguous names stay null. 1,425 of 1,695 player mentions matched (2026-09; the rest are mostly AHL players and prospects), up from 807 with `players` alone. A later round of wording fixes (missing "for", team names inside asset lists, "and traded him to", waiver claims skipped — see `trade_parse.py`) cut unrecognized assets across all 2,762 to 44. Full rebuild each night: upserts on `trade_id` / `(trade_id, idx)`, deletes trades that no longer exist.
...
**Model:** every remaining regular-season game from the live schedule (gameType 2, not yet OFF/FINAL — 1,344 games for 2026-27's 84-game season) is simulated with `elo.expected_prob()` on the two teams' current `team_elo_ratings`, plus `elo.HOME_ADVANTAGE` unless the schedule marks the game neutral-site. 22.1% of games go past regulation (loser gets a point) — pooled from `game_log` regular seasons 2023-24 through 2025-26 (20.9% / 20.7% / 24.8% by season; the backtest barely moves across that range). **Rating uncertainty:** each simulated season shifts every team's rating by its own random N(0, sd) amount for the whole season, with sd = `RATING_SD_PRESEASON` · √(`RATING_SD_HALF_GAMES` / (`RATING_SD_HALF_GAMES` + average games played)) — 50 Elo points preseason, ~36 by Jan 1, ~32 by Mar 1 (tuned with `backtest_playoff_odds.py --tune`). Without it, fixed ratings made the odds overconfident. Seeding: top 3 in each division, then the 2 best remaining teams per conference; ties broken by points, then regulation wins, then wins, then random (head-to-head and later NHL tiebreakers aren't modeled). Divisions/conferences come from the season's `team_seasons` rows, falling back to the prior season's alignment until current rows exist. Per team: `playoff_pct`, `division_pct`, `proj_points` with a 10th–90th percentile range.

**Explaining each day's change:** each run also stores every team's odds conditional on each result of every game on the next game-day (`playoff_odds_game_impacts`). The next run looks up those games' results and writes `change` on each team's row: `{prev_run_date, prev_pct, delta, contributions: [{game_id, home, away, winner, delta}], residual}`, where each contribution is (odds given the actual result) − (previous odds). The team's own games plus any other game worth ≥ 0.5 points are listed; `residual` is what the played games don't explain (rating changes, games further out, simulation noise).

**Backtest** (`backtest_playoff_odds.py` → `docs/playoff_odds_backtest_results.md`, read-only; `--tune` grid-searches the rating uncertainty): 2023-24 through 2025-26, four snapshots each — preseason (day before opening night, every team at 0 points; the standings endpoint returns no teams before a season starts), Nov 15, Jan 1, Mar 1 — with standings from the NHL's standings-by-date endpoint, Elo replayed to each date, scored against the 16 teams that actually made the playoffs. Overall Brier **0.161** with rating uncertainty vs **0.165** with fixed ratings vs **0.224** for a "currently holds a playoff spot" baseline (0.5 for everyone preseason). **Preseason odds are weak:** 0.251 / 0.211 / 0.258 across the three preseasons, where a flat 50% scores 0.250 — last season's ratings (even regressed) say little about who makes the playoffs. From Nov 15 on the model is clearly useful (0.08–0.22 vs 0.13–0.38 baseline). Consumers should label early-season odds as low-confidence, or show them only after ~10 games.

```bash
python playoff_odds.py                       # current season, 10,000 sims
python playoff_odds.py --sims 2000 --seed 42 --dry-run
python backtest_playoff_odds.py --sims 5000  # writes docs/playoff_odds_backtest_results.md
```
...
### `social_posts.py` (2026-09)

Automatic posts to Instagram and the EyeWall Facebook Page, run by `.github/workflows/social-posts.yml` (not `run.py`):
...
| `pwhl-winners` -- each PWHL game's projected winner + win probability | daily 15:45, skipped with no PWHL games | `pwhl_game_win_probs` (`hockeytech_elo.py`) |
...
| `goal-of-week` -- one goal from last Mon-Sun as a ~15 s Instagram Reel / Facebook Page video, redrawn from NHL player and puck tracking (see below) | Fri 17:00 | NHL game center (`landing`), each goal's tracking replay (`pptReplayUrl`), play-by-play for the still-card fallback |
...
`goal-of-week` lives in `goal_of_week.py` (added 2026-09). Only goals that decided or tied a game late are eligible: overtime goals, and go-ahead or tying goals in the last 5 minutes of the 3rd, never into an empty net. Among those it picks the goal whose puck travelled furthest in the 6 seconds before it went in, measured from the tracking replay (sampled every half second, so jitter doesn't pad a puck that sat still). Two 2025-26 weeks checked against real data had 23 and 12 eligible goals, every replay fetchable, and picked Brady Tkachuk (198 ft) and Alex Laferriere's go-ahead goal at 17:50 of the 3rd (230 ft). Which frame is the goal comes from the tracking itself (the puck entering the net from in front, or reappearing past the goal line after tracking lost it on the shot); checked against 40 goals' play-by-play shot spots, the tracked puck passes within 5 ft of the spot just before the detected goal frame in 37, and the other three are deflections or a shot the tracking lost. Tracking coordinates are inches from a corner; in play-by-play terms x ft = x/12 - 100 and y ft = 42.5 - y/12. The replay is every skater's and the puck's position at 10 Hz, in inches on a 200 x 85 ft rink, from the undocumented endpoint behind NHL.com's goal visualizer. It returns 403 unless the request carries a browser User-Agent, so it can disappear without notice. The clip is 1080x1920 at 30 fps (tracking interpolated), H.264 plus a silent AAC track, encoded by ffmpeg (both workflows make sure it's installed). The rink is turned so the attacked net is at the top, the camera follows the puck, and the clip ends on a card with the score, assists and the puck's path. If no eligible goal's replay can be fetched, the week falls back to a still 1080x1350 card instead: the goal is chosen by game situation alone (overtime winner, then the latest go-ahead goal, then the latest tying goal), drawn at its play-by-play shot location, and posted as an image. Regular season and playoffs; a week without games exits 0.

Cards are 1080x1350 JPEGs drawn with Pillow in the site's palette and Barlow fonts (`assets/fonts/`, OFL). They're uploaded to the public Supabase Storage bucket `social`, since both platforms fetch images by URL, and then published through the Graph API. Instagram gets a single image or a carousel. Facebook gets a photo post, or several unpublished photo uploads attached to one feed post. Video (`goal-of-week`) goes through `ship_video()` the same way: the MP4 is uploaded to the same bucket, then published as an Instagram Reel (also shown on the profile grid, with the cover frame at the goal) and a Facebook Page video posted by URL to `graph-video.facebook.com`. Instagram transcodes a Reel before it can publish, so its container is polled for up to 10 minutes rather than 2. Every attempt is written to `social_posts`, one row per platform. A `(platform, post key)` pair (post key = `<kind>-<ET date>`) that's already published is never posted again. That makes the backup crons safe: if one platform failed, the backup retries only that one. Facebook captions swap Instagram's "link in bio" for a real `eyewallanalytics.com` link.
...
- `ensure_season_row()` (2026-09) — runs first in both `run()` and `--game-log-only`: inserts the `pwhl_seasons` row for the season being processed if it's missing, from HockeyTech's `bootstrap` `seasons[]` entry (name, start date) via `build_season_row()`. `pwhl_game_log.season_id` has a foreign key to `pwhl_seasons`, which was only ever hand-seeded, so every new HockeyTech season_id used to fail with a `23503` FK violation until someone added the row. Never guesses — a season missing from bootstrap is logged and left unseeded, so the FK error still surfaces
...
**`--game-log-only [season_id]` — runs `fetch_game_log()` alone, skipping roster/player/goalie/team-stats.** Added for preseason: `run()`'s default `season_id` is always `PWHL_SEASON` (the live-resolved most recent *regular* season), so preseason's own `season_id` (10 = `2026-27 Pre-Season`, confirmed live 2026-09) is never swept by the main nightly step — same gap playoffs already had, which needed a manual per-round backfill. `pwhl-nightly.yml` now runs `python pwhl_stats.py --game-log-only 10` unconditionally every night rather than leaving it fully manual, since preseason (unlike a playoff round) is a month-long window with new results day to day. It was a no-op until the 2026-27 preseason schedule was published (2026-09), which then broke the step three nights running (2026-09-11 → 09-13) in two ways, both fixed: (1) HockeyTech sends `"-"` as the goal count for unplayed games — now parsed by `_goal_count()` as 0 (the game's state, not its score, marks it Final); (2) season 10 had no `pwhl_seasons` row — now seeded automatically by `ensure_season_row()` above. The same investigation found `SEASON_YEAR_MAP` (season → start year, used to add the year to HockeyTech's year-less schedule dates) missing season 10 and off by one for seasons 3 and 6, which had dated every 2023-24 and 2024-25 playoff game a year late; corrected and backfilled (`--game-log-only 3` / `6`), verified by matching every computed date's weekday against the weekday HockeyTech prints. Bump the hardcoded `10` once a year, same convention `eyewall-analytics`'s `pwhlConfig.js` uses for its own `PWHL_SEASONS` entries.
...
**Danger-zone values recalibrated (2026-08)**: the bucket values were originally ported verbatim from `rapm.py` -- described at the time as "NHL's own calibration," though it turned out in 2026-09 that those values had never been calibrated either, running about double the NHL's real per-band rates (see `nhl_shot_xg.py` above) -- unvalidated against PWHL's actual shot-danger distribution — this made absolute GSAX magnitude run high relative to NHL norms (+22 to +66 for current-season starters, vs NHL's typical -15 to +25). Fixed by computing real PWHL goal-conversion rates per bucket from 24,885 historical shot attempts across every season with shot-event coverage: high 0.14 (was 0.20), medium 0.08 (was 0.07), low 0.03 (unchanged). Same fix applied to `pwhl_shot_xg.py`'s skater-side `finishing` metric, which shares this constant. Production `gsax`/`finishing`/percentile columns backfilled with the recalibrated values.

**The real cause, fixed (2026-09)**: that recalibration didn't bring GSAX down (strong starters still read +40 to +56) because the bucket values weren't the problem. Both `pwhl_shot_xg.shot_xg()` and this module's `_shot_xg()` valued every goal at a flat 1.0. So a shooter's `xg_for` was goals + every miss's xG, about 2x the goals actually scored (1,081 vs 550 in 2025-26), and `finishing` was negative for every skater. A goalie's `gsax` was just the xG of every save: always positive, growing with workload. Goals are now valued by location like any other attempt. Shooters use each bucket's goal rate over all attempts (0.141 / 0.081 / 0.027). Goalies use the rate over shots that reached them (`DANGER_XG_FACED`: 0.148 / 0.101 / 0.043), since blocked shots never do. Both sum to within ~1% of actual goals over every season with shot data, and the tests pin that. 2025-26 after the fix: league xG 570 vs 549 goals; top finisher Pannek +9.6; top GSAX Desbiens +21.1.
...
All AHL modules use the same HockeyTech/LeagueStat vendor API as PWHL (`lscluster.hockeytech.com`) but a different client config (`client_code=ahl`, `key=ccb91f29d6744675`, `league_id=4`, `site_id=3`) and write to `ahl_*` Supabase tables. Dedicated tables rather than a shared NHL/PWHL-shaped schema with a league discriminator column — deliberate choice (see `docs/ahl_new_tables_ddl.sql`'s header comment): AHL's real data shape has no shift/TOI/attempts-based-Corsi/faceoff/hits data at all, so forcing it into PWHL's schema would mean permanently-null columns on every row. No `ahl_teams` table exists either — same as PWHL, team display metadata lives in the frontend, and `hockeytech_leagues.py` hardcodes each league's team map the same way `pwhl_stats.py` does. Full investigation notes and endpoint-by-endpoint findings: `docs/hockeytech-ahl-api-notes.md`.

**AHL and ECHL share one implementation.** Each `ahl_*.py`/`echl_*.py` script is a thin wrapper over a `hockeytech_*.py` module (`hockeytech_stats`, `_game_boxscore`, `_shot_events`, `_penalty_shots`, `_live_refresh`, `_news`), whose functions take a `League` config from `hockeytech_leagues.py` — keys, IDs, Referer, team maps, fallback season, news sources, and the one real difference in how the modules behave (ECHL's `players` view carries `team_name`, so skaters are matched to teams by name). The script names and arguments are unchanged. `test_hockeytech_characterization.py` runs every entry point for both leagues against fake feeds and compares the result to `tests/golden/hockeytech/`.

**The current season comes from the Worker's `/config/seasons`** through `season_lookup.get_hockeytech_season()`, the same Worker-backed pattern `pwhl_stats.py`/`nhl_stats.py` use, so the pipeline and the frontend agree and the Worker's `config:season:ahl:override` / `config:season:echl:override` KV keys steer both (env var `AHL_SEASON`/`ECHL_SEASON`, then 90/73, if the Worker is unreachable). Until 2026-09 the pipeline resolved this itself from HockeyTech's `seasons` feed. Any season's type and its start/end dates (the game-log date window) now come from the Worker too, via `/config/seasons/ahl-seasons` / `/echl-seasons` and `season_lookup.get_hockeytech_seasons()`, so the pipeline no longer reads that feed at all; if the Worker is unreachable it assumes a regular season and a wide date window, and logs it. The Worker picks the most recent `career="1"` season whose `start_date` has already passed — **not simply the max `season_id`**. This matters in practice: confirmed live 2026-08-29 that the feed's highest `career=1` season_id (94, "2026-27 Regular Season") has a `start_date` of 2026-10-02, still in the future — naively taking the max would return a season with zero games, the same mistake `docs/hockeytech-api-notes.md`'s PWHL section already documents avoiding. Every other AHL/ECHL module uses `hockeytech_stats.py`'s `resolve_current_season()`/`resolve_season_type()` rather than re-deriving season logic itself.
...
**A real historical franchise rename, not a bug:** Bridgeport Islanders (`team_id` 317, code `"BRI"`) relocated to become the Hamilton Hammers (`team_id` 457, code `"HAM"`) for 2026-27. `teamsbyseason` only ever returns each franchise's CURRENT identity, even when queried with an old `season_id` — so ingesting a historical season (e.g. 90, 2025-26) needs the OLD code merged in separately. These sit at the end of AHL's team map in `hockeytech_leagues.py` (currently just `317: "BRI"`), under a comment marking them as historical — add future renames there.
