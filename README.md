...
**The current season comes from the Worker's `/config/seasons`** through `season_lookup.get_hockeytech_season()`, the same Worker-backed pattern `pwhl_stats.py`/`nhl_stats.py` use, so the pipeline and the frontend agree and the Worker's `config:season:ahl:override` / `config:season:echl:override` KV keys steer both (env var `AHL_SEASON`/`ECHL_SEASON`, then 90/73, if the Worker is unreachable). Until 2026-09 the pipeline resolved this itself from HockeyTech's `seasons` feed; that feed is still used for the type of an arbitrary `season_id` and a season's date window, since the Worker has no AHL/ECHL equivalent of `/config/seasons/pwhl-types`. The Worker picks the most recent `career="1"` season whose `start_date` has already passed — **not simply the max `season_id`**. This matters in practice: confirmed live 2026-08-29 that the feed's highest `career=1` season_id (94, "2026-27 Regular Season") has a `start_date` of 2026-10-02, still in the future — naively taking the max would return a season with zero games, the same mistake `docs/hockeytech-api-notes.md`'s PWHL section already documents avoiding. Every other AHL/ECHL module uses `hockeytech_stats.py`'s `resolve_current_season()`/`resolve_season_type()` rather than re-deriving season logic itself.
