...
**AHL and ECHL share one implementation.** Each `ahl_*.py`/`echl_*.py` script is a thin wrapper over a `hockeytech_*.py` module (`hockeytech_stats`, `_game_boxscore`, `_shot_events`, `_penalty_shots`, `_live_refresh`, `_news`), whose functions take a `League` config from `hockeytech_leagues.py` — keys, IDs, Referer, team maps, fallback season, news sources, and the two real differences between the leagues (ECHL's name-based skater team lookup, and how strictly per-game responses are JSONP-unwrapped). The script names and arguments are unchanged. `test_hockeytech_characterization.py` runs every entry point for both leagues against fake feeds and compares the result to `tests/golden/hockeytech/`.
