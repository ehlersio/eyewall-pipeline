...
**Model:** every remaining regular-season game from the live schedule (gameType 2, not yet OFF/FINAL — 1,344 games for 2026-27's 84-game season) is simulated with `elo.expected_prob()` on the two teams' current `team_elo_ratings`, plus `elo.HOME_ADVANTAGE` unless the schedule marks the game neutral-site. 24.8% of games go past regulation (loser gets a point) — measured from 2025-26, the only season with reliable `period_end` data. **Rating uncertainty:** each simulated season shifts every team's rating by its own random N(0, sd) amount for the whole season, with sd = `RATING_SD_PRESEASON` · √(`RATING_SD_HALF_GAMES` / (`RATING_SD_HALF_GAMES` + average games played)) — 50 Elo points preseason, ~36 by Jan 1, ~32 by Mar 1 (tuned with `backtest_playoff_odds.py --tune`). Without it, fixed ratings made the odds overconfident. Seeding: top 3 in each division, then the 2 best remaining teams per conference; ties broken by points, then regulation wins, then wins, then random (head-to-head and later NHL tiebreakers aren't modeled). Divisions/conferences come from the season's `team_seasons` rows, falling back to the prior season's alignment until current rows exist. Per team: `playoff_pct`, `division_pct`, `proj_points` with a 10th–90th percentile range.
