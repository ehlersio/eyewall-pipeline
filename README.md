...
**Model:** every remaining regular-season game from the live schedule (gameType 2, not yet OFF/FINAL — 1,344 games for 2026-27's 84-game season) is simulated with `elo.expected_prob()` on the two teams' current `team_elo_ratings`, plus `elo.HOME_ADVANTAGE` unless the schedule marks the game neutral-site. 24.8% of games go past regulation (loser gets a point) — measured from 2025-26, the only season with reliable `period_end` data. Ratings are held fixed within a simulated season. Seeding: top 3 in each division, then the 2 best remaining teams per conference; ties broken by points, then regulation wins, then wins, then random (head-to-head and later NHL tiebreakers aren't modeled). Divisions/conferences come from the season's `team_seasons` rows, falling back to the prior season's alignment until current rows exist. Per team: `playoff_pct`, `division_pct`, `proj_points` with a 10th–90th percentile range.
...
**Backtest** (`backtest_playoff_odds.py` → `docs/playoff_odds_backtest_results.md`, read-only): 2023-24 through 2025-26, snapshots Nov 15 / Jan 1 / Mar 1, standings from the NHL's standings-by-date endpoint, Elo replayed to each date, scored against the 16 teams that actually made the playoffs. Brier score **0.136** vs **0.215** for a "currently holds a playoff spot" baseline, better at all 9 snapshots. Well calibrated at the extremes (teams given 90–100% made it 97% of the time; 0–10% made it 4%). **Known limitation:** somewhat overconfident in the middle (80–90% predictions made it 72% of the time; 50–60% made it 45%) because fixed ratings ignore rating uncertainty — most visible preseason (COL 95.0% before 2026-27's first game). Adding per-simulation rating uncertainty that shrinks as the season progresses, tuned with an Oct 1 backtest snapshot, is the planned follow-up.
