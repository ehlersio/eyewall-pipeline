"""
hockeytech_shot_events.py -- AHL/ECHL shot events with coordinates, from
play-by-play. Shared by ahl_shot_events.py and echl_shot_events.py; every
entry point takes the league config (hockeytech_leagues.py) first.

Endpoint: statviewfeed / gameCenterPlayByPlay -- a flat list of
{event, details} dicts.

Event types captured:
  shot  -- shooter, goalie, xLocation/yLocation, shotType, shotQuality,
           isGoal. Only ingested when isGoal is falsy -- see `goal`.
  goal  -- a DISTINCT event type from `shot`, unlike PWHL (whose goals are
           only `shot` events with isGoal=true). It carries assists[],
           properties (isPowerPlay/isShortHanded/isEmptyNet/isPenaltyShot/
           isInsuranceGoal/isGameWinningGoal) and plus/minus players
           directly, so no gameSummary merge is needed, unlike
           pwhl_shot_events.py. Confirmed across 16+ real games in both
           leagues that shot+isGoal and `goal` describe the same goal, with
           `goal` a strict superset -- a clean disjoint pair.

  blocked_shot/hit/faceoff: NOT present in either league's PBP at all. Don't
  add handling without first seeing them in a live pull.
  penaltyshot: no coordinates, make or miss -- see hockeytech_penalty_shots.py.

Coordinate transform: PWHL's CANVAS_W/CANVAS_H=600x300 and home-attacks-
right-in-odd-periods fold, unmodified -- sample games in both leagues land in
the same x/y range as PWHL's.
"""
...
def _pipeline(lg: League) -> str:
    return f"{lg.key}_shot_events"


def transform_coords(x_raw: int, y_raw: int, is_home: bool, period: int) -> tuple:
    """pwhl_shot_events.py's transform -- same canvas, same fold."""
...
def fetch_pbp(lg: League, game_id: int, retries: int = 3) -> list | None:
    """Play-by-play events for one game. Raises FetchError after exhausting
    retries -- a real fetch failure, distinct from a game legitimately
    having no PBP (an empty list is a normal response)."""
    last_err = None
    ...
    for ev in events:
        ...
        if event_type == "shot":
            if bool(d.get("isGoal", False)):
                continue  # the parallel `goal` event owns this one
...
def ingest_game(lg: League, sb, gid: int, home_id: int, season_id: str, season_type: str) -> int:
    events = fetch_pbp(lg, gid)
    if not events:
        log.warning("    No PBP -- skipping")
        _mark_skipped(lg, sb, gid, "no_pbp")
        return 0

    rows = parse_pbp(gid, season_id, season_type, home_id, events)
    if not rows:
        log.info("    No shot events")
        _mark_skipped(lg, sb, gid, "no_shots")
        return 0

    # Stub any player referenced in a shot/goal who isn't in {league}_players
    # yet (a call-up who's in neither the roster nor the league leaders can
    # still take a shot).
    player_ids = set()
    for row in rows:
    ...
    # Deduplicate within this game before upserting: two shot events can
    # share (game_id, event_type, period_id, time_seconds, team_id,
    # shooter_id) when a player has two shots in the same recorded second,
    # which crashes a single-statement upsert (Postgres 21000, "ON CONFLICT
    # DO UPDATE command cannot affect row a second time" -- hit in 5 of 89
    # games of one production season). x_raw/y_raw disambiguate: a real
    # duplicate has identical coordinates and still collapses; two genuinely
    # different shots almost always differ in location.
    seen = set()
    deduped = []
    for row in rows:
        key = (
    ...
    for j in range(0, len(deduped), 200):
        sb.table(f"{lg.key}_shot_events").upsert(
            deduped[j : j + 200],
...
def run(lg: League, season_id: str | None = None) -> None:
    ...
    log.info(f"=== {lg.label} Shot Events -- season {season_id} ({season_type}) ===")
    ...
    for i, game in enumerate(todo):
        ...
        try:
            ingest_game(lg, sb, gid, home_id, season_id, season_type)
    ...
    log.info(f"=== {lg.label} Shot Events complete ===")
...
def run_single_game(lg: League, game_id: int) -> None:
    ...
    log.info(f"=== {lg.label} Shot Events -- single game {game_id} (season {season_id}) ===")
    ingest_game(lg, sb, game_id, home_id, season_id, season_type)
    log.info("=== Done ===")
