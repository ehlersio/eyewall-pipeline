...
- `hockeytech_stats.py` (AHL/ECHL, and every `ahl_*`/`echl_*` module through it) — `resolve_current_season()` reads the Worker's `ahl`/`echl` entries via `get_hockeytech_season()` (2026-09). `resolve_season_type()` and `_season_day_window()` read the Worker's `/config/seasons/{ahl,echl}-seasons` via `get_hockeytech_seasons()` (2026-09), so nothing reads HockeyTech's `seasons` feed directly anymore. `get_hockeytech_seasons()` returns `None` (not a guess) when the Worker is unreachable; those two then fall back to `"regular"` and a wide date window, logged.
...
Each of the four modules above now has a local `_resolve_season_type(season_id)` helper: **hardcoded `SEASON_TYPE_MAP` first, `get_season_type()` as the live fallback.** This is deliberate, not a straight replacement — the hardcoded map holds a manual correction (season "2" — see "PWHL season 2 is the 2023 showcase" below) that live bootstrap data would silently overwrite if trusted outright. When neither source recognizes an id, behavior splits by call-site shape:
...
## Known open items
- ~~Season ID 2 naming discrepancy~~ — resolved 2026-09, see "PWHL season 2 is the 2023 showcase" below.
...
- `pwhl_goal_on_ice.py`/`pwhl_penalty_shots.py` (Session 42) are still manual-only, not wired into `pwhl-nightly.yml` — same gap `pwhl_game_boxscore.py` had until Session 50 fixed it for that one module.
- `inspect_*`/`test_*` diagnostic files are intentionally kept around (not scratch files to delete) — not audited recently though.

## PWHL season 2 is the 2023 showcase — keep `SEASON_TYPE_MAP`'s label
HockeyTech's `bootstrap` calls season 2 `"2024 Preseason"`; `SEASON_TYPE_MAP` calls it `"showcase"`, and the map is right. Its games settle it: 9 games over 2023-12-04 to 12-07, all six teams three apiece, before the inaugural regular season (ID 1) opened 2024-01-01. Resolved 2026-09 (README has the detail).

It's also the one PWHL season whose feed calls Montréal `MON` rather than `MTL` — mapped by `TEAM_CODE_ALIASES` in `pwhl_stats.py`. Go through `team_id_for()` for any feed `team_code`, never a reverse scan of `TEAM_ID_MAP`: an unplaced code must be skipped and logged, not written as `team_id` NULL. NULL never matches the season tables' conflict keys, so every run inserts another copy (how season 2 reached 96 duplicate rows by 2026-06, and ECHL's Iowa/Utah 356).
...
## Supabase returns at most 1,000 rows per request — page every multi-row read

A bare `sb.table(...).select(...).execute()` silently stops at the project's row cap (1,000 today; 999 at one point, see `moneypuck.py`). No error, just a truncated list. Found 2026-09 in the per-game pipelines' `get_completed_games`/`get_processed_games`/`get_skipped_games` (AHL/ECHL `hockeytech_*` and the five PWHL modules): "completed" saw only 1,000 of AHL's 1,152 regular-season games (152 never got box scores, 127 no shot events; ECHL 80 of 1,080), and "processed" reads one row per player/event, so it recognised only ~20–30 games as done and every run re-ingested nearly the whole season. Fixed with `pipeline_common.select_all(lambda: <query>)`, which pages with `.order().range()` until an empty page. Use it (or an existing `.range()` loop like `shift_data.py`'s) for any read that can exceed 1,000 rows. A test fake for such a read must honour `.range()`; `select_all` raises after 1,000 pages instead of looping forever on one that doesn't.

## `rapm.py`/`score_state.py` shift_events query — needs a *covering* index, not just a composite one
Session 47 diagnosed `rapm.py`'s league-wide, season-only `shift_events` keyset query as prone to `57014` statement timeouts even with cursor pagination, and proposed a composite `(season, id)` index as the fix. Session 50 found that index alone doesn't work: Postgres's planner kept choosing `shift_events_pkey` (scan by `id`, filter by `season`) over it, discarding 1M+ non-matching rows before finding a match — a genuine planner blind spot, not stale stats (`ANALYZE` made no difference) or query shape (`ORDER BY season, id` didn't help either). Root cause: `season = 20232024`'s matching rows are clustered at the high end of the `id` range (backfilled after other seasons already had lower ids), so Postgres's uniform-distribution cost assumption for `ORDER BY id LIMIT 999` badly underestimates the pkey-scan-then-filter plan's real cost. The actual fix was making the index **covering** (`INCLUDE`-ing the queried columns) so Postgres can do an `Index Only Scan` — that cost advantage was enough to flip the planner's choice; no code changes needed. See `docs/session47_shift_events_index.sql` for the full diagnosis and the exact index. If a future `57014` shows up on a different table/query, check `EXPLAIN (ANALYZE, BUFFERS)` for which index is actually chosen before assuming an index is missing — it may already exist and just not be picked.
