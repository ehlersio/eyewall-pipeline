...
def _parse_faceoff(
    ...
    player_team_map: dict | None = None,
) -> dict:
...
def parse_pbp(
    ...
    for ev in events:
        ...
        if event_type == "hit":
            rows.append(
                _parse_hit(d, game_id, int(season_id), season_type, period, time_seconds, team_id)
            )
        elif event_type == "penalty":
            rows.append(
                _parse_penalty(
                    d, game_id, int(season_id), season_type, period, time_seconds, team_id
                )
            )
...
def get_processed_games(sb, season_id: str) -> set:
    result = sb.table("pwhl_pbp_events").select("game_id").eq("season_id", int(season_id)).execute()
    return {r["game_id"] for r in (result.data or [])}
...
def ingest_game(
    ...
    rows = parse_pbp(
        gid, season_id, season_type, home_team_id, events, away_team_id, player_team_map
    )
    if not rows:
