...
def get_processed_games(sb, season_id: str) -> set:
    result = sb.table("pwhl_shot_events").select("game_id").eq("season_id", int(season_id)).execute()
    return {r["game_id"] for r in (result.data or [])}
...
def parse_pbp(game_id: int, season_id: str, season_type: str,
    ...
    for ev in events:
        ...
        period = period_map.get(str(period_raw)) or int(period_raw) if str(period_raw).isdigit() else period_map.get(str(period_raw), 4)
    ...
    if TRANSFORM_DEBUG and debug_coords:
        ...
        log.info(f"  [DEBUG] x_raw: {min(x_raws)}-{max(x_raws)}, y_raw: {min(y_raws)}-{max(y_raws)}")
        log.info(f"  [DEBUG] x_norm: {min(x_norms):.1f}-{max(x_norms):.1f}, y_norm: {min(y_norms):.1f}-{max(y_norms):.1f}")
    ...
    for i, game in enumerate(todo):
        ...
        if not events:
            ...
            sb.table("pwhl_skipped_games").upsert({
                "game_id": gid, "pipeline": pipeline, "reason": "no_pbp",
        ...
        if not rows:
            ...
            sb.table("pwhl_skipped_games").upsert({
                "game_id": gid, "pipeline": pipeline, "reason": "no_shots",
                "skipped_at": datetime.now(UTC).isoformat(),
            }, on_conflict="game_id,pipeline").execute()
            continue
        ...
        if player_ids:
            existing = sb.table("pwhl_players").select("player_id").in_("player_id", list(player_ids)).execute()
        ...
        deduped = []
        for row in rows:
            key = (row["game_id"], row["event_type"], row["period_id"],
                   row["time_seconds"], row["team_id"], row["shooter_id"])
            if key not in seen:
                seen.add(key)
                deduped.append(row)
