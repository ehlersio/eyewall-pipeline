...
def run_internal_checks(client, season, our_rapm):
    ...
    prior_rows = fetch_all(
        client, "player_seasons", "player_id,rapm", {"season": prior_season_id, "game_type": 2}
    )
...
def run_eh_comparison(client, season, our_rapm, eh_csv_path):
    ...
    try:
        with open(eh_csv_path, encoding="utf-8-sig") as f:
    ...
    if not id_col or not rapm_col:
        print("  ERROR: Could not identify player_id or RAPM column.")
        print(f"  Available columns: {list(eh_rows[0].keys())}")
        return None, None, ["Could not parse EH CSV — check column names"]
    ...
    if outliers:
        ...
        for o in outliers[:5]:
            print(
                f"    {o['player_id']}: ours={o['our_rapm']:+.3f}, EH={o['eh_rapm']:+.3f}, delta={o['delta']:+.3f}"
            )
...
def run(season=NHL_SEASON, eh_csv_path=None):
    ...
    rows = fetch_all(client, "player_seasons", "player_id,rapm", {"season": season, "game_type": 2})
