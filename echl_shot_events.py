"""
echl_shot_events.py — ECHL shot event pipeline module

Fetches play-by-play for all completed ECHL games via the
gameCenterPlayByPlay endpoint and extracts shot attempts and goals with
coordinates. Structurally mirrors ahl_shot_events.py (same vendor, same
PBP schema, confirmed live 2026-08-30 against game 24296 -- 81 events:
goalie_change x4, shot x59, penalty x7, goal x11).

Endpoint: statviewfeed / gameCenterPlayByPlay on feed/index.php
Response: flat list of {event, details} dicts

Event types captured (same disjoint shot/goal pair as AHL -- see
ahl_shot_events.py's docstring for the full reasoning, unchanged here):
  shot  — only ingested when isGoal is falsy.
  goal  — a distinct event type, already carries assists[]/properties/
          plus_players/minus_players directly, no gameSummary merge
          needed.

  (blocked_shot, hit, faceoff: confirmed NOT present in ECHL's PBP either
   -- same wall as AHL.)
  (penaltyshot events intentionally NOT parsed here -- no coordinates
   exist for them; see echl_penalty_shots.py)

Coordinate transform: reuses AHL/PWHL's CANVAS_W/CANVAS_H=600x300 and
transform_coords() fold unmodified -- ECHL's own coordinate sample
(confirmed live: x∈[36,579], y∈[12,282] across the original
investigation) matches the same range. Do one visual overlay check
against a real rink before fully trusting this on a shot map.
...
# Same canvas AHL/PWHL use -- see module docstring.
CANVAS_W = 600.0
CANVAS_H = 300.0
...
PIPELINE = "echl_shot_events"


def transform_coords(x_raw: int, y_raw: int, is_home: bool, period: int) -> tuple:
    """Unmodified copy of ahl_shot_events.py's transform."""
...
def fetch_pbp(game_id: int, retries: int = 3) -> list | None:
    """Fetch play-by-play events for a single game. Same JSONP-unwrap
    hardening as echl_stats.py's _modulekit_get()/echl_game_boxscore.py's
    _hockeytech_get() -- only strip parens when the response actually
    starts/ends with them (confirmed live this endpoint IS genuinely
    JSONP-wrapped, same guard applied defensively regardless)."""
    last_err = None
...
def ingest_game(sb, gid: int, home_id: int, season_id: str, season_type: str) -> int:
    ...
    # Same dedupe-by-coordinates key AHL's own pipeline needed in
    # production (see docs/echl_new_tables_ddl.sql's natural key comment)
    # -- built in from day one here rather than rediscovered.
    seen = set()
    deduped = []
    for row in rows:
        key = (
    ...
    for j in range(0, len(deduped), 200):
        sb.table("echl_shot_events").upsert(
            deduped[j : j + 200],
...
def run(season_id: str | None = None) -> None:
    ...
    log.info(f"=== ECHL Shot Events -- season {season_id} ({season_type}) ===")
    ...
    log.info("=== ECHL Shot Events complete ===")
...
def run_single_game(game_id: int) -> None:
    ...
    log.info(f"=== ECHL Shot Events -- single game {game_id} (season {season_id}) ===")
