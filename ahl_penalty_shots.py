"""
ahl_penalty_shots.py — AHL penalty shot pipeline module

Fetches play-by-play for each completed AHL game and extracts the
`penaltyshot` event (make or miss) into ahl_penalty_shots.

Why the PBP `penaltyshot` event directly, unlike PWHL's
pwhl_penalty_shots.py (which deliberately avoids the PBP `penaltyshot`
event in favor of gameSummary's penaltyShots[] key -- see that module's
docstring): AHL/ECHL's PBP `penaltyshot` event already has both makes and
misses, a fully-resolved shooter_team object, and shooter/goalie as full
player objects -- confirmed live (2026-08-29). PWHL's PBP version of this
event has a thinner team object, which is why PWHL's pipeline goes to
gameSummary instead; that reason doesn't apply here. No gameSummary fetch
is needed for this table at all.

Confirmed: no coordinate data exists for penalty shots (make or miss) --
same as PWHL, this table intentionally has no x/y columns, and these
events are NOT written to ahl_shot_events (a coordinate-based shot-map
table; penalty shots don't belong there).

This module does its own independent PBP fetch rather than importing
ahl_shot_events.fetch_pbp, matching this codebase's existing convention of
pipeline modules that parse the same feed independently rather than
coupling to each other (see pwhl_penalty_shots.py's _hockeytech_get
docstring). The extra per-game fetch this costs is the same tradeoff
pwhl_penalty_shots.py already accepts.
...
def get_processed_games(sb, season_id: str) -> set:
    result = (
        sb.table("ahl_penalty_shots").select("game_id").eq("season_id", int(season_id)).execute()
    )
    return {r["game_id"] for r in (result.data or [])}
...
def ingest_game(sb, gid: int, season_id: str, season_type: str) -> int:
    ...
    sb.table("ahl_penalty_shots").upsert(
        rows,
...
def run(season_id: str | None = None) -> None:
    ...
    log.info(f"=== AHL Penalty Shots -- season {season_id} ({season_type}) ===")
    ...
    log.info(f"=== AHL Penalty Shots complete -- {total} row(s) upserted ===")
...
def run_single_game(game_id: int) -> None:
    ...
    log.info(f"=== AHL Penalty Shots -- single game {game_id} (season {season_id}) ===")
    ingest_game(sb, game_id, season_id, season_type)
    log.info("=== Done ===")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="AHL penalty shot pipeline")
